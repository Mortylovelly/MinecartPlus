...
public class AdvancedMinecartRenderer extends GeoEntityRenderer<AdvancedMinecartEntity> {
    ...
    @Override
    protected void applyRotations(AdvancedMinecartEntity entity, MatrixStack matrices,
                                  float ageInTicks, float rotationYaw,
                                  float partialTick, float nativeScale) {
        float yaw = MathHelper.lerpAngleDegrees(partialTick, entity.prevYaw, entity.getYaw());
        Vec3d velocity = entity.getVelocity();
        double horizontalSpeed = Math.sqrt(velocity.x * velocity.x + velocity.z * velocity.z);
        float pitch = 0.0F;
        if (horizontalSpeed > 1.0E-4D) {
            pitch = (float) (-Math.toDegrees(Math.atan2(velocity.y, horizontalSpeed)));
        }
        matrices.multiply(RotationAxis.POSITIVE_Y.rotationDegrees(270.0F - yaw));
